...
- 用户反馈：
  ...
  - 首页“第一次来，可以先从这些活动看起”与独立活动页重复。
  - 这两处都应删除。
- 已完成：
  - `HeroSection` 删除顶部重复 logo 与 `heroKicker` 文案。
  - Hero 向下箭头从 `#activity-preview` 改为 `#mission`，避免指向已删除区块。
  - 首页 `src/app/page.tsx` 移除 `ActivityPreviewSection`。
  - 删除已无引用的 `src/components/landing/ActivityPreviewSection.tsx`。
  - 中文/日文首页文案同步清理：
    - 删除 `heroKicker`
    - 删除整组 `activityPreview*`
    - 删除已无引用的首页 `navTeam` / `team*`
    - `heroGuide` 改成不再提“点左边”，而是直接引导去活动页
- GLM 复核结论：
  - 结论：**合理**
  - 理由：
    - 首页聚焦 Hero 定位与 CTA，噪音更低
    - 页面节奏变成“定位 → 怎么参加 → 信任建立”，更顺
  - 风险：
    - 活动预览完全移除后，用户对“当前活动活跃度”的直观感知会降低
  - 建议但**本轮未采纳**：
    - 后续可在 Hero 或数据区加入一条极轻量的近期活动亮点或动态数字
- 本轮验证结果：
  - `node JSON.parse zh/ja messages`：通过
  - `pnpm typecheck`：通过
  - `pnpm lint`：通过
  ...
  - 生产 smoke：
    - 首页不再出现 `东京各校学生一起玩的地方`
    - 首页不再出现 `第一次来，可以先从这些活动看起`
    - 首页不再出现 `组织的人`
    - `/scripts` 页面仍正常打开

---

## 04-24 玩家首页与组织介绍页拆分补记
- 用户要求：
  - 折纸竹鹤再设计得更像“折纸鹤”，不要太像抽象叶片。
  - “组织活动的人”不要继续放在玩家首页，应移到另一个对外介绍页，主要给合作方/赞助方/投资视角的人看。
  - 让 GLM 复核这个产品改动是否合理。
- 已完成：
  - `OrigamiCraneIcon` 重画为更明确的折纸鹤结构：双翼、颈部、喙、尾部、折线和纸面层次更明显。
  - `OrigamiCraneLauncher` 的入口底座从深色圆底改为浅色纸感圆底，提升折纸鹤识别度。
  - 首页 `src/app/page.tsx` 移除 `StaffSection`，玩家首页不再展示团队介绍。
  - `LandingNav` 移除“组织的人 / 運営メンバー”导航入口。
  - 新增独立页面 `src/app/organization/page.tsx`，作为对外介绍页承载组织结构与核心成员。
  - `StaffSection` 改为可传入标题文案的通用组件，供组织页复用。
  - 首页信息结构现为：Hero → 精选活动 → 怎么参加 → 数据 → 成员评论 → FAQ → 联系。
- GLM 复核结论：
  - 结论：**合理**
  - 理由：
    - 首页聚焦首访者核心任务“先看活动”
    - Staff 信息与普通玩家路径解耦，更符合信息分层
    - 动画改为彩蛋入口，减少对转化的阻塞
  - 风险：
    - 极少数强依赖“谁在办”的用户，首页信任背书会变弱
    - 折纸竹鹤入口可能被部分用户忽略
  - 建议但**本轮未采纳**：
    - Hero 里可加一条极轻量的组织背书小链接，指向 `/organization`
- 本轮验证结果：
  - `node JSON.parse zh/ja messages`：通过
  - `pnpm typecheck`：通过
  - `pnpm lint`：通过
  ...
  - `pnpm build`：通过
  - 浏览器 smoke：
    - 首页不再出现 Staff 区块和对应导航
    - 右下角折纸竹鹤入口可见
    - `/organization` 页面可正常打开并显示组织介绍与成员卡片
...
- 用户反馈：
  - 新版首页和上一版方案差异不够明显。
  - 首屏右侧说明区块不太需要。
  - 多处文案像“对设计者/产品评审说话”，不是对访问者/玩家说话。
  - 期望改成学生社团宣传口吻，不要太官方。
- 已修复：
  - `HeroSection` 去掉右侧独立说明卡，改为一整块校园图 + 深绿遮罩的完整宣传 Hero。
  - 首屏文案改为“东京周末，一起找点好玩的事”，面向同学邀请，而不是产品定位说明。
  - 首页中文/日文文案整体降官方感：
    - 活动区改成“第一次来，可以先从这些活动看起”
    - Mission 改成“想认识新朋友，不一定要硬聊”
    - 数据区改成“来之前，你可以先知道这里大概是什么样”
    - BrandMotion 改成“从一场活动开始，慢慢认识更多人”
    - Staff 改成“这里不是自动运转的系统，是一群同学在认真组局”
    - FAQ 改成“你可能会先想问这些”
- 本轮验证结果：
  - `node JSON.parse zh/ja messages`：通过
  - `pnpm typecheck`：通过
  ...
  - `pnpm build`：通过
  - `pnpm test:unit`：78/78 通过
  - Chrome production screenshot：`output/frontend-visitor-tone-home.png`，确认首屏不再有右侧说明区块，文案更像对学生访问者说话。
