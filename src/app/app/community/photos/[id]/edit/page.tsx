...
export default async function EditPhotoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!detail || detail.post.postType !== "photo" || !detail.post.isMine) notFound()
  if (!context.profile) redirect("/app/profile/community?setup=1")
  return <PhotoComposer profile={context.profile} locale={normalizeCommunityLocale(rawLocale)} post={detail.post} />
}
