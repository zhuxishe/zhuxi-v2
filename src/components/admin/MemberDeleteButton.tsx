...
export function MemberDeleteButton({ memberId, memberName, accountStatus, anonymizedAt }: Props) {
  ...
  function changeStatus(status: "active" | "suspended" | "closed", label: string) {
    const warning = status === "closed"
      ? "关闭是终态，之后不能通过状态 RPC 重新启用。关闭不会自动匿名化资料。确认继续吗？"
      : `确认${label}该账号吗？`
  ...
  function anonymize() {
    const warning = anonymizedAt
      ? "数据库资料已匿名化；本次只重试封禁/删除残留 Auth 用户并写入完成标记。确认继续吗？"
      : "匿名化会改写个人资料、解除 canonical user_id、关闭账号并删除 Auth 用户。此操作不是简单删除，也不能通过本页撤销。确认继续吗？"
    if (!window.confirm(warning)) return
    setError(null)
    setMessage(null)
    startTransition(async () => {
  ...
  return (
    <section className="rounded-xl border border-amber-200 bg-amber-50 p-5" aria-labelledby="member-lifecycle-title">
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-2 text-xs leading-5 text-amber-900">
              当前本库状态：<strong>{accountStatus ?? "未设置（null）"}</strong>；匿名化时间：<strong>{anonymizedAt ?? "未匿名化（null）"}</strong>。
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 rounded-lg border border-amber-300 bg-white p-3 text-xs leading-5 text-amber-950">
        <strong>Supabase Auth 联动：</strong>若成员绑定了 Auth user_id，本页会先通过 Auth Admin API 执行 ban/unban，再调用带管理员 session 的数据库 RPC；数据库失败时会尝试恢复操作前的 Auth 状态，补偿失败会明确报错且绝不按成功展示。
        暂停、恢复与关闭不会删除 Auth 用户；匿名化会在数据库完成资料匿名化并解除 canonical user_id 后，硬删除 Auth 用户。删除失败时孤立账号会保持封禁并提示人工重试。
        Auth ban 不主动撤销既有 session，因此 closed/anonymized 的数据库状态、路由检查与 RLS 仍是最终访问防线。未绑定 Auth 的成员只更新本站数据库。
      </div>
      ...
      {impact ? (
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <div className="rounded-lg border border-amber-200 bg-white p-3"><p className="text-xs font-semibold text-amber-900">阻断条件 / Blockers</p><div className="mt-2 text-sm"><ImpactValue value={impact.blockers} /></div></div>
          <p className="md:col-span-2 text-xs text-amber-900">
            ...
            Auth 清理待完成：{impact.auth_operation_required === true ? "是" : impact.auth_operation_required === false ? "否" : "未返回"}。
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <label className="block">
          <span className="text-xs font-semibold text-amber-950">匿名化 / 硬删除二次确认</span>
          <input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={`输入完整 canonical ID：${memberId}`} className="mt-1 min-h-10 w-full rounded-lg border border-amber-300 bg-white px-3 text-sm outline-none focus:ring-2 focus:ring-amber-400" disabled={pending} />
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        {Boolean(anonymizedAt) && impact?.auth_operation_required === true ? <Button type="button" variant="destructive" onClick={anonymize} disabled={!ready || confirmation.trim() !== memberId}><UserX className="size-4" />重试 Auth 删除收口</Button> : null}
