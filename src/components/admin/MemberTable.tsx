...
export function MemberTable({ members, canViewHighRisk, redactedFields }: Props) {
  if (members.length === 0) {
    return <p className="py-12 text-center text-sm text-muted-foreground">没有符合条件的用户 / 成员</p>
  }
    ...
    <div className="overflow-x-auto">
      {!highRiskVisible ? (
        <p className="border-b border-border bg-amber-50 px-4 py-2 text-xs text-amber-900">
          权限隐藏：会员编号、Auth email 与登录 provider 仅 super_admin 可见；这不代表对应业务值为 null。
        </p>
      ) : null}
      <table className="w-full min-w-[1180px] text-sm">
        <thead>
          <tr className="border-b border-border text-left">
            <th className="px-4 py-3 font-medium text-muted-foreground">用户 / 成员</th>
            <th className="px-4 py-3 font-medium text-muted-foreground">Canonical member ID</th>
            <th className="px-4 py-3 font-medium text-muted-foreground">Auth 绑定</th>
          ...
          {members.map((member) => (
            <tr key={member.memberId} className="border-b border-border/50 align-top transition-colors hover:bg-muted/30">
              ...
              <td className="px-4 py-3">
                ...
                ) : <p className="mt-1 text-xs text-amber-700">provider 因权限隐藏</p>}
              </td>
              <td className="px-4 py-3">
              ...
              <td className="px-4 py-3 font-mono text-xs">{highRiskVisible ? member.memberNumber ?? "未分配（null）" : <span className="font-sans text-amber-700">因权限隐藏</span>}</td>
              <td className="px-4 py-3 text-xs text-muted-foreground">{formatDate(member.updatedAt)}</td>
              <td className="px-4 py-3 text-right">
                <Link href={`/admin/members/${member.memberId}`} className="font-medium text-primary hover:underline">查看 360</Link>
              </td>
            </tr>
          ))}
        </tbody>
